...
def main_control_loop():
    print(f"Pętla z tłumieniem")
    motor_on_of(True)

    current_torque_nm=0
    motor_angle_degress = set_torque_and_get_angle(0.0)
    if motor_angle_degress is None:
        print ("Błą komunikacji"); motor_on_of(False); bus.shutdown(); return
    ...
    try:
        while True:
            ...
            print(f"Kąt:{output_angle_degress:6.2f}° | Moment G:{gravity_torque_nm:.3f}| Moment D:{damping_torque_nm:.3f}| Zadanu: {current_torque_nm:.3f} Nm", end='\r')
            time.sleep(0.01)
        ...
        print("\nWyłączam silnik")
        set_torque_and_get_angle(0); motor_on_of(False); bus.shutdown()
